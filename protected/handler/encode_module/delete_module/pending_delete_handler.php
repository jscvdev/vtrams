...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    require_once 'pending_delete.ctrl.inc.php';
    require_once 'pending_delete.model.inc.php';
    ...
    try {
        ...
        try {
            if (isset($_REQUEST['delete_document'])) {
                ...
                $result = is_pending_delete_required_data_empty($variables_to_check);
                ...
                if (check_exists_in_pending($pdo, $encoded_document_id)) {
                    $temp_dump['document_exists'] = "Document does not exist!";
                }

                //CHECK ANY VALIDATION FAILS ELSE PROCEED TO EXECUTE DATABASE QUERY
                if ($temp_dump) {
                    $_SESSION['error_pending_delete'] = $temp_dump;
                    echo "<script>process_functionAlert('Delete Failed!', 'delete_pending_process_err')</script>";
                    die();
                } else {
                    // DATABASE STATEMENTS VIA MODE/CTRL
                    delete_target_logs($pdo, $encoded_document_id);
                    log_user_action(
                        $pdo,
                        $encoded_document_id,
                        $encoded_document_title,
                    ...
                    echo "<script>process_functionAlert('Delete success!', 'delete_pending_success')</script>";
                }
            } else {
                echo "<script>process_functionAlert('Delete Error: Wrong module used!', 'delete_document_err')</script>";
            }
