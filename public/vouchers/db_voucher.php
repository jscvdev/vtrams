...
function dv_fetch_all_signatories(PDO $pdo, ?string $office = null): array
{
    ...
    return utilities_build_dv_signatory_client_payload($pdo, $resolvedOffice);
}

$dv_signatory_client = (isset($pdo) && $pdo instanceof PDO)
    ...
    : [
        ...
        'defaultIds' => ['cert' => 0, 'accounting' => 0, 'approved' => 0],
        'printable' => false,
    ];
$dv_signatory_labels = $dv_signatory_client['labels'] ?? utilities_dv_signatory_labels_map();
$dv_signatory_roles = $dv_signatory_client['roles'] ?? utilities_dv_signatory_roles_map();
$dv_default_cert_key = (string) ($dv_signatory_client['defaultCertKey'] ?? 'dv_certified_msd');
...
<?php $dvFormDataJsVer = is_file(__DIR__ . '/../../protected/js/fetch_voucher_form_data.js') ? (int) filemtime(__DIR__ . '/../../protected/js/fetch_voucher_form_data.js') : time(); ?>
<script src="../../protected/js/fetch_voucher_form_data.js?v=<?= $dvFormDataJsVer ?>"></script>
<script>
    ...
    window.DV_SIGNATORY = <?= json_encode(array_merge($dv_signatory_client, [
...
<div class="printableTable" id="printableTable" style="margin-top: 20px;">
    <table class="db-voucher__table" id="my-Table" border="1" cellspacing="0" cellpadding="0">
        ...
        <tbody>
            ...
            <tr>
                <th colspan="5" class="pad-2">E. Receipt of Payment</th>
            </tr>
            <tr>
                <th class="text-centered pad-5">Check/ADA No.</th>
                <td class="pad-5"></td>
                <td class="pad-5">Date:</td>
                <td class="pad-5" style="height: 50px">
                    <div style="height: 100% !important; width: 100%;white-space: nowrap; padding: 0; display: flex; flex-direction: column;">
                        <p style="margin-bottom: 3%">Bank Name & Account Number:</p>
                    </div>
                </td>
                <td class="pad-5" style="height: 50px">
                    <div style="height: 100% !important; width: 100%;white-space: nowrap; padding: -5px 0 0 0; display: flex; flex-direction: column;">
                        <p style="margin-bottom: 3%">JEV No.</p>
                    </div>
                </td>
            </tr>
            <tr>
