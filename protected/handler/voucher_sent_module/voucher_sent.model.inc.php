...
string $receiver_udc, string $encoded_by, string $encoded_from, string $datetime_encoded, string $process_status, string $combined_remarks, string $file_path = '') {
    ...
    // Preserve gross/charged from sent; POST amount may be the effective net value.
    $selectQuery = 'SELECT amount, charged_amount, supporting_documents FROM voucher_sent WHERE processing_no = :processing_no';
