...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    require_once 'voucher_ada_remove.ctrl.php';
    require_once 'voucher_ada_remove.model.php';
    ...
    try {
        $temp_dump = [];
        try {
            if (isset($_REQUEST['remove_voucher'])) {
                ...
                if ($temp_dump) {
                    $_SESSION['error_voucher_remove'] = $temp_dump;
                    echo "<script>process_functionAlert('Remove failed!', 'voucher_ada_redirect')</script>";
                    die();
                } else {
                    // DATABASE STATEMENTS VIA MODE/CTRL
                    if (check_if_voucher_added_exists($pdo, $processing_no)) {
                        voucher_remove_voucher($pdo, $processing_no, $dv_no);
                        voucher_remove_from_temp(
                            $pdo,
                            $processing_no,
                            $ors_no,
                            $ada_check_no,
                        ...
                        echo "<script>process_functionAlert('Remove success!', 'voucher_ada_redirect')</script>";
                        die();
                    } else {
                        voucher_remove_voucher($pdo, $processing_no, $dv_no);
                        voucher_remove_from_temp(
                ...
                echo "<script>process_functionAlert('Remove Error: Wrong module used!', 'voucher_ada_redirect')</script>";
                die();
            }
