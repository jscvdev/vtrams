    ...
    try {
        ...
        try {
            if (isset($_REQUEST['edit_document'])) {
                ...
                //QUERY
                $query = "UPDATE routing SET document_title = :document_title, document_desc = :document_desc, document_receive_type = :document_receive_type, document_type = :document_type, 
                ...
                if ($statement->execute()) {
                    echo "<script>process_functionAlert('Edit success!', 'edit_routing_redirect')</script>";
                    die();
                }
            } else {
                echo "<script>process_functionAlert('Edit error: Wrong Module Used!', 'edit_routing_redirect')</script>";
                die();
            }
