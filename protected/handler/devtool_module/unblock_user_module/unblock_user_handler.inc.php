...
try {
    if ($emp_id === '') {
        echo "<script>process_functionAlert('Unblock Failed: invalid employee ID.', 'developer_edit_err')</script>";
        die();
    }

    if (!user_login_unblock($pdo, $emp_id)) {
        echo "<script>process_functionAlert('Unblock Failed: user not found.', 'developer_edit_err')</script>";
        die();
    }

    echo "<script>process_functionAlert('User unblocked successfully.', 'developer_edit_success')</script>";
    die();
} catch (PDOException $e) {
    echo "<script>process_functionAlert('Unblock Failed.', 'developer_edit_err')</script>";
    die();
}
